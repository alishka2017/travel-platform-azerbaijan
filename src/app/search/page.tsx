...
function SearchContent() {
  ...
  const allResults = query ? searchPlaces(query) : places.slice(0, 12);
  const filteredResults = filter === 'all' ? allResults : allResults.filter(r => r.category === filter);
      ...
      <div className="mb-8">
        ...
        {query && (
          <p className="text-gray-600">
            Found {allResults.length} results for <span className="font-semibold">"{query}"</span>
          </p>
        )}
      </div>
